...
async function addBuildToProject(wsConfig, context, projectName) {
    let result = await (0, build_selector_1.buildSelector)(context, wsConfig);
...
async function addRunnerToBuild(wsConfig, context, projectName, buildName) {
    ...
    if (path.isAbsolute(build.relBoardSubDir)) {
        result = await (0, runner_selector_1.runnerSelector)(build.relBoardSubDir);
    }
    else {
        result = await (0, runner_selector_1.runnerSelector)(path.join(wsConfig.rootPath, build.relBoardDir, build.relBoardSubDir)); // Will remove eventually
    }
    if (result && result.name !== undefined) {
...
//# sourceMappingURL=project%20copy.js.map