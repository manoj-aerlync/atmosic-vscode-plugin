    ...
    if (useExternalJson) {
        ...
        try {
            if (!fs.pathExistsSync(zephyrIdeSettingFilePath)) {
                await fs.writeFile(zephyrIdeSettingFilePath, JSON.stringify({}, null, 2), { flag: 'w+' }, function (err) {
                    if (err) {
                        throw err;
                    }
...
async function setWorkspaceState(context, wsConfig) {
    fs.writeFile(path.join(wsConfig.rootPath, ".vscode/zephyr-ide.json"), JSON.stringify({ projects: wsConfig.projects }, null, 2), { flag: 'w+' }, function (err) {
        if (err) {
            throw err;
        }
    });
...
exports.toolsdir = path.join(os.homedir(), toolsfoldername);
exports.toolchainDir = path.join(exports.toolsdir, "toolchains");
...
async function setupWestEnvironment(context, wsConfig, globalConfig, solo = true) {
    ...
    await vscode.window.withProgress({
        ...
        // Install `west`
        res = await (0, utils_1.executeShellCommand)(`${python} -m pip install west`, wsConfig.activeSetupState.setupPath, (0, utils_1.getShellEnvironment)(wsConfig.activeSetupState), true);
        if (res.stdout) {
            utils_1.output.append(res.stdout);
...
//# sourceMappingURL=setup%20copy.js.map